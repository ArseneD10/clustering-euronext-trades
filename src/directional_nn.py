...
class DiscreteHead(nn.Module):
    ...
    def __call__(self, cfeatures: mx.ArrayLike, categorical_features: Dict[str, mx.ArrayLike]):
        x = self.encoder(features=cfeatures, categorical_features=categorical_features).mean(axis=1)
        x = self.head(x)
        return x
